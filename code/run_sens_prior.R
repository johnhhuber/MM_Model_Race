...
# multiply upper and lower bounds by 2 
params_lower <- params_lower * 1.5
params_upper <- params_upper * 1.5

bayesianSetup <- createBayesianSetup(ll_cmp, lower = params_lower, upper = params_upper)
settings = list(iterations = 2e6, message = T)
...
# write to file 
write.csv(post, file = paste('../output/sens_prior/posterior_', chain_id, '.csv', sep = ''), row.names = F)
